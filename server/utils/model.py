...
class PersonDetector(OpenvinoModel):
    ...
    def __process_results(self, h: int, w: int, results, thresh: float = 0.5):
        """
        추론 결과 처리
        openvino notebook에서 가져온 코드

        Args:
            h (int): 원본 이미지 높이
            w (int): 원본 이미지 너비
            results (np.ndarray): 추론 결과
            thresh (float): 임계값
...
class PoseEstimator(OpenvinoModel):
    ...
    def visualize(self, frame: np.ndarray, keypoints: np.ndarray) -> np.ndarray:
        """
        추론 결과를 이용해 skeleton 이미지 생성

        Args:
            frame (np.ndarray): 원본 이미지
            keypoints (np.ndarray): 키 포인트

        Returns:
            np.ndarray: 생성된 skeleton 이미지
        """
